...
enum Bytecode {
#define __(A, B, C) A,
  VCL_BYTECODE_LIST(__) SIZE_OF_BYTECODE
#undef __  // __
};
...
class BytecodeBuffer {
 ...
 public:
  ...
  inline void Emit(const vcl::util::CodeLocation& loc, Bytecode op) {
    ...
    if ((m_size + 1) > m_capacity) Reserve(m_capacity * 2, 1);
  ...
  inline void Emit(const vcl::util::CodeLocation& loc, Bytecode op,
    ...
    if ((m_size + 4) > m_capacity) Reserve(m_capacity * 2, 4);
    m_buffer[m_size] = static_cast<uint8_t>(op);
    Emit(oper, m_buffer + m_size + 1);
    m_size += 4;
  }

  static bool CheckOperand(uint32_t operand) { return operand < kMaxArg; }

#define IMPL0(TYPE, BC)                                            \
  void TYPE(const vcl::util::CodeLocation& loc) { Emit(loc, BC); } \
  void TYPE() { Emit(vcl::util::CodeLocation(), BC); }
...
#undef IMPL1  // IMPL1
 ...
 public:  // Jump and label handling
  struct Label {
    Label(uint32_t position, BytecodeBuffer* bb)
        : m_position(position), m_patched(false), m_bbuffer(bb) {}

    Label() : m_position(0), m_patched(false), m_bbuffer(NULL) {}
    ...
    bool IsPatched() { return m_patched; }
 ...
 public:  // Accessors
  ...
  int position() const { return static_cast<int>(m_size); }
  ...
  class Iterator {
   public:
    Iterator(const Iterator& itr)
        : m_bb(itr.m_bb), m_index(itr.m_index), m_arg(itr.m_arg) {}
    ...
    Bytecode operator*() const { return bytecode(); }
    ...
    uint32_t arg() const {
      ...
      m_arg = 3;
      uint32_t ret = static_cast<uint32_t>((cp3 << 16) | (cp2 << 8) | (cp1));
   ...
   private:
    ...
    Iterator(const BytecodeBuffer* bb) : m_bb(bb), m_index(0), m_arg(0) {}

    Iterator(const BytecodeBuffer* bb, size_t idx)
        : m_bb(bb), m_index(idx), m_arg(0) {}
 ...
 private:
  ...
  size_t Put(const vcl::util::CodeLocation& loc, Bytecode op) {
    ...
    if (m_size + 4 > m_capacity) Reserve(2 * m_capacity, 4);
    m_buffer[m_size] = static_cast<uint8_t>(op);

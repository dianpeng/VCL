...
void Parser::ParserError( const char* format , ... ) {
  ...
  if(m_lexer.lexeme().token == TK_ERROR) {
    ...
    *m_error = ReportErrorV(m_source,m_lexer.location(),"grammar",format,vl);
  }
}
  ...
  while(m_lexer.lexeme().token != TK_EOF) {
    ...
    switch(m_lexer.lexeme().token) {
      ...
      case TK_EOF:
        break;
      default:
        ParserError("In global scope, you are only allowed to put statement "
...
ast::AST* Parser::ParseGlobalVariable() {
  ...
  if(!m_lexer.Try(TK_VAR)) {
    ParserError("global variable expect a variable name after "
                "\"global\" keyword!");
    return NULL;
  }
  ...
  if(!m_lexer.Try(TK_ASSIGN)) {
    ParserError("global variable assignment expect a \"=\" after variable "
                "name!");
    return NULL;
  }
  m_lexer.Next(); // Skip =
  ...
  if(m_lexer.Next().token == TK_LBRA) {
    if(m_lexer.Next().token == TK_RBRA) {
      ...
      do {
        ...
        switch(m_lexer.lexeme().token) {
          case TK_STRING:
            item.name = zone::ZoneString::New(m_zone,m_lexer.lexeme().string());
            m_lexer.Next();
            break;
          case TK_NOT:
            item.negative = true;
            if(!m_lexer.Try(TK_STRING)) {
              ParserError("Expect a string literal to serve IP address in ACL!");
              return NULL;
            }
            item.name = zone::ZoneString::New(m_zone,m_lexer.lexeme().string());
            m_lexer.Next();
            break;
          default:
            ParserError("In ACL list,you are only allowed to put quoted string "
...
ast::AST* Parser::ParseExtension() {
  ...
  zone::ZoneString* type_name = zone::ZoneString::New(m_zone,
      m_lexer.lexeme().string());
  if(!m_lexer.Try(TK_VAR)) {
    ParserError("Expect a variable name after the extension type name!");
    return NULL;
  }
  zone::ZoneString* instance_name = zone::ZoneString::New(m_zone,
      m_lexer.lexeme().string());
...
ast::AST* Parser::ParseSub() {
  ...
  zone::ZoneString* name = zone::ZoneString::New(m_zone,m_lexer.lexeme().string());
  m_lexer.Next();
...
ast::AST* Parser::ParseAnonymousSub() {
  ...
  m_lexer.Next();
  if(m_lexer.lexeme().token != TK_LPAR &&
     m_lexer.lexeme().token != TK_LBRA) {
    ParserError("Anonymouse sub routine requires a optional argument list or "
...
ast::AST* Parser::ParseInclude() {
  ...
  zone::ZoneString* path = zone::ZoneString::New( m_zone ,
      m_lexer.lexeme().string() );
  if(!m_lexer.Try(TK_SEMICOLON)) {
...
ast::AST* Parser::ParseImport() {
  ...
  zone::ZoneString* name = zone::ZoneString::New( m_zone ,
      m_lexer.lexeme().string() );
  if(!m_lexer.Try(TK_SEMICOLON)) {
...
ast::AST* Parser::ParseStatement() {
  switch(m_lexer.lexeme().token) {
    case TK_LBRA: return ParseLexScope();
    ...
    case TK_SET: return ParseSet();
    case TK_UNSET: return ParseUnset();
    case TK_NEW: return ParseNew();
    case TK_DECLARE: return ParseDeclare();
    case TK_CALL: return ParseCall();
    case TK_RETURN: return ParseReturnOrTerminate();
    case TK_VAR: return ParsePrefixCall();
    default:
      ParserError("expect a valid statement here, a statment can be "
                  "return,if,set,unset,new,declare,call,member function call!");
      return NULL;
  }

loop_error:
  ParserError("In this script configuration, we don't support for/break/continue "
              "language constructs!");
  return NULL;
}
...
ast::AST* Parser::ParseStatementWithSemicolon() {
  ...
  if(!ret) return NULL;
  if((ret->type != ast::AST_IF &&
      ret->type != ast::AST_LEXSCOPE &&
...
ast::AST* Parser::ParseFor() {
  ...
  zone::ZoneString* var = zone::ZoneString::New( m_zone, m_lexer.lexeme().string());
...
ast::AST* Parser::ParseSet() {
  ...
  switch(m_lexer.lexeme().token) {
    ...
    case TK_SELF_MOD:
      break;
    default:
      ParserError("In set statement, expected operators are \"=\",\"+=\""
          ",\"-=\",\"*=\",\"/=\",\"%-\"!");
      return NULL;
  }
...
bool Parser::ParseLHS( ast::LeftHandSide* output ) {
  if(m_lexer.lexeme().token != TK_VAR) {
    ...
    zone::ZoneString* prefix = zone::ZoneString::New(
...
ast::AST* Parser::ParsePrefixCall() {
  DCHECK(m_lexer.lexeme().token == TK_VAR);
  zone::ZoneString* prefix = zone::ZoneString::New( m_zone ,
      m_lexer.lexeme().string() );
  m_lexer.Next();
  int last_component = ast::Prefix::Component::DOT;
  ast::Prefix* ret;
...
ast::AST* Parser::ParseCall() {
  ...
  if(m_lexer.Next().token != TK_VAR) {
    ParserError("Call statement must follow a variable name indicate "
...
ast::AST* Parser::ParseTerminate() {
  ...
  if(m_lexer.Next().token == TK_RPAR) {
    ...
    if(m_lexer.lexeme().token == TK_VAR) {
      ...
      ParserError("Expect special *variable name* in return statement with"
          " terminate semantic!");
      return NULL;
    }
    if((terminate->action = vcl::GetActionNameEnum(prefix->data())) ==
      ...
      if(IsPrefixOperator(m_lexer.lexeme().token)) {
        ...
        if(!(terminate->value = ParsePrefix(prefix,&op)))
          return NULL;
...
fail:
        ParserError("In terminated return statement, you can only put "
            "action name or function call!");
        return NULL;
      }
    }
    if(!m_lexer.Expect(TK_RPAR)) {
      ParserError("Expect \")\" to close return statement with "
          "terminate semantic!");
      return NULL;
    }
    return terminate;
...
ast::AST* Parser::ParseTernary() {
  DCHECK( m_lexer.lexeme().token == TK_IF );
  ast::Ternary* ternary = new (m_zone) ast::Ternary(
      m_lexer.location());
...
ast::AST* Parser::ParseBinaryPrecedence( int precedence ) {
  if(precedence <0) return ParseUnary();
  else {
    DCHECK( static_cast<size_t>(precedence) < boost::size(kPrecedence) );
    ast::AST* left;
    if(!(left = ParseBinaryPrecedence(precedence-1)))
      return NULL;
    ...
    do {
      if(TokenIsBinaryOperator(m_lexer.lexeme().token)) {
        ...
        if(cur_precedence == precedence) {
          ...
          Token op = m_lexer.lexeme().token; m_lexer.Next();
...
ast::AST* Parser::ParseUnary() {
  if(m_lexer.lexeme().token != TK_ADD &&
     m_lexer.lexeme().token != TK_SUB &&
...
ast::ExtensionLiteral* Parser::ParseExtensionLiteral(
    zone::ZoneString* prefix ) {
  ast::ExtensionLiteral* ret = new (m_zone)
    ast::ExtensionLiteral(m_lexer.location());
...
ast::ExtensionInitializer* Parser::ParseExtensionInitializer() {
  ...
  while( m_lexer.lexeme().token != TK_RBRA &&
    ...
    if(!m_lexer.Expect(TK_DOT) || m_lexer.lexeme().token != TK_VAR) {
      ParserError("Field inside of extension must have leading \".\" followed "
    ...
    if(!m_lexer.Try(TK_ASSIGN)) {
      ParserError("Expect a \"=\" for field assignment in extension initializer!");
      return NULL;
    }
    m_lexer.Next();
  ...
  if(m_lexer.Next().token == TK_RPAR) {
    ...
    do {
      ...
      if(m_lexer.lexeme().token == TK_COMMA) {
        ...
        ParserError("Expect a \",\" or \")\" here in function call argument list ",
      ...
      case TK_COLON:
        {
          ...
          m_lexer.Next();
          if(last_component)
            *last_component = ast::Prefix::Component::DOT;
          break;
        }
      case TK_LSQR:
        m_lexer.Next();
        ...
        if(last_component)
          *last_component = ast::Prefix::Component::INDEX;
        break;
      case TK_LPAR:
        {
...
ast::AST* Parser::ParsePrimary() {
  ...
  switch(m_lexer.lexeme().token) {
    case TK_INTEGER:
      ret = new (m_zone) ast::Integer(m_lexer.location(),m_lexer.lexeme().integer());
      m_lexer.Next();
      return ret;
    case TK_REAL:
    ...
    case TK_NULL:
      ...
      return ret;
    case TK_DURATION:
      ret = new (m_zone) ast::Duration(m_lexer.location(),m_lexer.lexeme().duration());
      m_lexer.Next();
      return ret;
    case TK_SIZE:
...
ast::AST* Parser::ParseAtomic() {
  ...
  if(primary) return primary;
    ...
    switch(m_lexer.lexeme().token) {
      ...
      case TK_LSQR: return ParseList();
      case TK_LBRA: return ParseDict();
      case TK_LPAR: {
        // Subexpression
        m_lexer.Next();
      ...
      default:
        ParserError("unrecognize expression, expect a primary expression or a "
...
ast::AST* Parser::ParseStringInterpolation() {
  ...
  m_lexer.Next();
  ast::StringInterpolation* interp = new (m_zone) ast::StringInterpolation(
      m_lexer.location());
  do {
    switch(m_lexer.lexeme().token) {
      case TK_INTERP_END:
        m_lexer.Next(); goto done;
      case TK_CODE_START: {
        ...
        if(m_lexer.lexeme().token != TK_RBRA) {
          ParserError("Expect a \"}\" to close a code segment inside of string "
                      "interpolation!");
          return NULL;
        }

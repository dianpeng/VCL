...
// separately. We know VCL has concatenation for function/sub , these concatenation
...
class Zone {
 public:
  ...
  size_t total_segment_size() const {
    return m_total_segment_size;
  }
 ...
 public:
  ...
  T Realloc( void* old , size_t old_size , size_t new_size ) {
    if(old_size >= new_size) return static_cast<T>(old);
    else {
...
class ZoneString : public ZoneObject {
 public:
  ...
  ZoneString( const ZoneString& rhs ):
    m_data(rhs.m_data),
    m_size(rhs.m_size)
  {}

  ZoneString( Zone* zone , const char* str ):
    m_data(NULL),
    m_size(strlen(str))
  {
  ...
  static ZoneString* New( Zone* zone , const char* str ) {
    ...
    return ::new (buf) ZoneString(str,
        static_cast<void*>(
          (static_cast<char*>(buf) + sizeof(ZoneString))),
        len);
  }
 ...
 private: // New interfaces used constructor supposed to be little faster
  ZoneString( const char* str , void* buf , size_t len ):
    m_data(static_cast<const char*>(buf)),
    m_size(len)
  { memcpy(buf,str,m_size); ((char*)(m_data))[m_size] = 0; }

  ZoneString( const std::string& str , void* buf ):
    m_data(static_cast<const char*>(buf)),
    m_size(str.size())
  { memcpy(buf,str.c_str(),m_size); ((char*)(m_data))[m_size] = 0; }
...
class ZoneVector : public ZoneObject {
 ...
 public:
  ...
  static ZoneVector* New( Zone* zone ) {
    return new (zone) ZoneVector();
  }

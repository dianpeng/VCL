...
template<typename T,typename E> class ByteArray {
 ...
 public:
  ...
  template< size_t INDEX > uint8_t Index() const {
...
bool IsValidIPAddress( const char* string ) {
  if(strcmp(string,"localhost") == 0) return true;
  else {
...
template< typename E , size_t BASE > inline E ToDigit( char c ) {
  if(BASE == 10) {
...
Tokenizer<E,BOUND,BASE>::Next() {
  ...
  switch(c) {
    case '.': YIELD(TOKEN_DOT,1);
    case ':': YIELD(TOKEN_COLON,1);
    case '[': YIELD(TOKEN_LSQR,1);
    case ']': YIELD(TOKEN_RSQR,1);
    case '-': YIELD(TOKEN_DASH,1);
    case '*': YIELD(TOKEN_STAR,1);
    case '0':case '1':case '2':case '3':case '4':
    case '5':case '6':case '7':case '8':case '9':
    ...
    case 'f': case 'F':
      ...
      return ScanComponent();
    case 0: YIELD(TOKEN_EOF,0);
    default: YIELD(TOKEN_ERROR,0);
  }
}
...
//    0 components.
// 3. All the IPV4 and IPV6 will be compiled into an object called IPMatchProgram,
...
// The 2 number represents the *start* and the *end* of this range. In IPV6, each
...
struct InstructionType {
  ...
  InstructionType( uint8_t op ):
    opcode(op),
    arg1(0),
    arg2(0)
  {}

  InstructionType( uint8_t op , uint16_t a1 ):
    opcode(op),
    arg1(a1),
    arg2(0)
  {}
  ...
  for( size_t i = 0 ; i < Program::kComponentSize ; ++i ) {
    ...
    switch(instr.opcode) {
      ...
      case ANY:
        ...
        break;
      case RANGE:
        if(instr.arg1 <= address[i] && address[i] <= instr.arg2)
          return false;
        break;
      case ZRANGE:
        {
...
struct Compiler {
 private:
  ...
  // ipv6 have condensed string representation. To support this kind of variance,
  // we will compile our string representation into a intermediate representation
  // and then later convert this intermediate representation into the
  // IPMatchProgram.
  enum {
    OP_V6_CONDENSED = SIZE_OF_OPCODE + 1
  };
  ...
  // cannot do it efficiently , so we may end up with some intermediate representation,
  // but using this function to *categorize* will be fast. Since the following
  // loop will be unrolled and most case we will just hit in very few instructions.
      ...
      case TOKEN_LSQR:
        {
          ...
          if(tokenizer->Next().token != TOKEN_COMPONENT)
            return false;
          ...
          if(tokenizer->Next().token != TOKEN_RSQR)
            return false;
          tokenizer->Next();
  ...
  bool DoCompile( const char* pattern , Program* ptr ) {
    T tokenizer(pattern); tokenizer.Next();
    ...
    for( size_t i = 0 ; i < ComponentSize - 1 ; ++i ) {
      ...
      if(!CompileComponent<T,IPV4,Wildcard>(
            &tokenizer,&(ptr->code_buffer[index])))
        return false;
      ...
      if(tokenizer.lexeme().token != delimiter)
        tokenizer.Next();
    ...
    if(tokenizer.lexeme().token != TOKEN_EOF)
      return false;
  ...
  bool EnsureMaskV6( IPV6Program* program , uint8_t mask ) {
    ...
    switch(mask) {
      ...
      case 48:size = 3; break;
      case 64:size = 4; break;
      case 80:size = 5; break;
      case 96:size = 6; break;
      case 112:size= 7; break;
      case 128:size= 8; break;
      default: return false;
    }
 ...
 public:
  ...
  bool CompileWildcard( const char* pattern , IPProgram* pt ) {
    ...
    switch(type) {
      case INVALID: return false; // Unknown pattern
      case IPV4: return CompileAsIPV4<true>(pattern,pt);
      case IPV6: return CompileAsIPV6<true>(pattern,pt);
  ...
  bool MatchImpl( E addr ) const {
    for( size_t i = 0 ; i < program_list.size() ; ++i ) {
      ...
      if(prg.which() == PROGRAM_INDEX) {
        if(DoMatch(boost::get<PT>(prg),B(addr)))
          return true;
      }
    }
    return false;
...
bool IPPatternImpl::Compile( const ast::ACL& acl_node ) {
  ...
  for( size_t i = 0 ; i < acl_node.list.size(); ++i ) {
    ...
    if(item.mask != 0) {
      ...
      if(!compiler.CompileWildcard(item.name->data(),&program))
        return false;

...
namespace {
#define __(A,B) B,
static const char* kActionStringTable [] = {
  VCL_ACTION_LIST(__)
  NULL
};
#undef __ // __
...
SourceCode* SourceRepo::FindOrLoadSourceCode( const std::string& file_path ,
  ...
  if(itr == m_source_code_table.end()) {
    ...
    if(m_interface) {
      if(!m_interface->LoadFile(file_path,&pu->source_code_info->source_code)) {
        *error = (boost::format("cannot load source file from path %s")%(file_path)).str();
        return NULL;
      }
    } else if(!vcl::util::LoadFile(file_path,&pu->source_code_info->source_code)) {
      *error = (boost::format("cannot load source file from path %s")%(file_path)).str();
      return NULL;
    }

    { // Do the parsing
      Parser parser(&m_zone,file_path,pu->source_code_info->source_code,error,
                     m_rand_name_seed,
                     m_allow_loop);
      if(!(pu->root = parser.DoParse())) {
        return NULL;
...
vm::Procedure* CompiledCodeBuilder::CreateSubRoutine( const vm::ast::Sub& sub ,
  ...
  m_cc->m_sub_routine_list.push_back( new vm::Procedure(
        ...
        sub.arg_list.size()));
  if(index) *index = static_cast<uint32_t>(
      m_cc->m_sub_routine_list.size()-1);
...
vm::Procedure* InternalAllocator::NewEntryProcedure() {
  vm::Procedure* procedure = new vm::Procedure(kEntryProcName,
                                               kEntryProcProtocol,
                                               0);
  return procedure;
}

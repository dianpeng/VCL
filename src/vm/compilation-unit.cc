...
class IncludeGuard {
 public:
  IncludeGuard( CompilationUnitBuilder* builder , const std::string& path ):
    ...
    m_fail(false)
  {
    std::set<std::string>::iterator itr = m_builder->m_include_stack.find(m_path);
...
class PUPusher {
 public:
    ...
    m_old_source_code_index( builder->m_cur_source_index )
  {
    ...
    builder->m_cur_source_index = builder->m_cc->AddSourceCodeInfo(
        pu->source_code_info);
  }
...
void CompilationUnitBuilder::ReportError( const vcl::util::CodeLocation& loc ,
  ...
  *m_error = vcl::util::ReportErrorV(
      m_cur_pu->source_code_info->source_code,
      loc,
      "[compilation-unit]",
      format,
      vl);
...
CompilationUnit::SubList*
CompilationUnitBuilder::AddSub( const std::string& sub_name ) {
  SubListIndex::iterator itr = m_sub_index.find(sub_name);
  if(itr == m_sub_index.end()) {
    CompilationUnit::SubListPtr sub_list = boost::make_shared<
      CompilationUnit::SubList>();
...
bool CompilationUnitBuilder::DoExpand( const File& root ) {
  for( size_t i = 0 ; i < root.chunk->statement_list.size() ; ++i ) {
    ...
    switch(ast->type) {
      case AST_INCLUDE:
        if(!DoInclude(static_cast<const Include&>(*ast)))
          return false;
        break;
      case AST_SUB:
        if(!DoSub(static_cast<const Sub&>(*ast)))
          return false;
        break;
      default:
        m_cu->m_statement.push_back(
...
bool CompilationUnitBuilder::DoSub( const Sub& sub ) {
  CompilationUnit::SubList* sub_list = AddSub( std::string(sub.sub_name->data()) );
  if(!sub_list->empty()) {
    ...
    if(fail) {
      ReportError(sub.location,"Sub %s, when merge other same name sub from "

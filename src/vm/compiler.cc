...
class Compiler {
 ...
 private:
  ...
  enum { CATEGORY_PROP, CATEGORY_ATTR, CATEGORY_INDEX, SIZE_OF_CATEGORY };
  ...
  template <typename T>
  int CompileString(const vcl::util::CodeLocation& loc, const T& value) {
  ...
  template <typename T>
  int CompileLiteral(const vcl::util::CodeLocation& loc, const T& value) {
  ...
  bool CompileVariable(const vcl::util::CodeLocation& loc, zone::ZoneString*);

 private:
  void ReportError(const vcl::util::CodeLocation& loc, const char* format, ...);
...
class LexicalScope {
 ...
 public:
  ...
  int base() const { return m_base + static_cast<int>(m_var.size()); }

 public:
  bool DefineIterator(Zone*);
  enum { LOCAL_TOO_MUCH = -1, LOCAL_DUPLICATE = -2 };
 ...
 public:  // Loop control related
  BytecodeBuffer::Label AddBreak(BytecodeBuffer* bb,
  ...
  BytecodeBuffer::Label AddContinue(BytecodeBuffer* bb,
                                    const vcl::util::CodeLocation& loc) {
...
int LexicalScope::LookupLocal(ZoneString* zone_string) {
  for (size_t i = 0; i < m_var.size(); ++i) {
    if ((*m_var[i]) == *zone_string) return static_cast<int>(i) + m_base;
  }
  return -1;
}
...
LexicalScope::~LexicalScope() {
  if (m_var.size() && !m_no_pop) PopAllVariablesInScope();
...
bool Compiler::Compile(const ast::ExtensionInitializer& initializer) {
  for (size_t i = 0; i < initializer.list.size(); ++i) {
    ...
    // Value
    if (!Compile(*field.value)) return false;
  }
  __ lext(initializer.location, static_cast<uint32_t>(initializer.list.size()));
  return true;
}
  ...
  for (size_t i = 0; i < unary.ops.size(); ++i) {
    ...
    switch (tk) {
      case TK_ADD:
        break;
      case TK_SUB:
        __ negate(unary.location);
        break;
      case TK_NOT:
        __ flip(unary.location);
        break;
      default:
        VCL_UNREACHABLE();
        break;
...
bool Compiler::Compile(const ast::Binary& binary) {
  if (TokenIsLogicOperator(binary.op)) {
    ...
    if (binary.lhs->type == ast::AST_INTEGER) {
      ...
      int index =
          CompileLiteral(binary.lhs->location,
                         static_cast<const ast::Integer*>(binary.lhs)->value);
      if (index < 0) return false;
      switch (binary.op) {
        case TK_ADD:
          __ addiv(binary.location, index);
          break;
        case TK_SUB:
          __ subiv(binary.location, index);
          break;
        case TK_MUL:
          __ muliv(binary.location, index);
          break;
        case TK_DIV:
          __ diviv(binary.location, index);
          break;
        case TK_MOD:
          __ modiv(binary.location, index);
          break;
        case TK_LT:
          __ ltiv(binary.location, index);
          break;
        case TK_LE:
          __ leiv(binary.location, index);
          break;
        case TK_GT:
          __ gtiv(binary.location, index);
          break;
        case TK_GE:
          __ geiv(binary.location, index);
          break;
        case TK_EQ:
          __ eqiv(binary.location, index);
          break;
        case TK_NE:
          __ neiv(binary.location, index);
          break;
        default:
          ReportError(binary.location,
                      "match/unmatch operator used with integer type");
          return false;
      }
    } else if (binary.rhs->type == ast::AST_INTEGER) {
      if (!Compile(*binary.lhs)) return false;
      int index =
          CompileLiteral(binary.rhs->location,
                         static_cast<const ast::Integer*>(binary.rhs)->value);
      if (index < 0) return false;
      switch (binary.op) {
        case TK_ADD:
          __ addvi(binary.location, index);
          break;
        case TK_SUB:
          __ subvi(binary.location, index);
          break;
        case TK_MUL:
          __ mulvi(binary.location, index);
          break;
        case TK_DIV:
          __ divvi(binary.location, index);
          break;
        case TK_MOD:
          __ modvi(binary.location, index);
          break;
        case TK_LT:
          __ ltvi(binary.location, index);
          break;
        case TK_LE:
          __ levi(binary.location, index);
          break;
        case TK_GT:
          __ gtvi(binary.location, index);
          break;
        case TK_GE:
          __ gevi(binary.location, index);
          break;
        case TK_EQ:
          __ eqvi(binary.location, index);
          break;
        case TK_NE:
          __ nevi(binary.location, index);
          break;
        default:
          ReportError(binary.location,
                      "match/unmatch operator used with integer type");
      ...
      switch (binary.op) {
        case TK_ADD:
          __ add(binary.location);
          break;
        case TK_SUB:
          __ sub(binary.location);
          break;
        case TK_MUL:
          __ mul(binary.location);
          break;
        case TK_DIV:
          __ div(binary.location);
          break;
        case TK_MOD:
          __ mod(binary.location);
          break;
        case TK_LT:
          __ lt(binary.location);
          break;
        case TK_LE:
          __ le(binary.location);
          break;
        case TK_GT:
          __ gt(binary.location);
          break;
        case TK_GE:
          __ ge(binary.location);
          break;
        case TK_EQ:
          __ eq(binary.location);
          break;
        case TK_NE:
          __ ne(binary.location);
          break;
        case TK_MATCH:
          __ match(binary.location);
          break;
        case TK_NOT_MATCH:
          __ nmatch(binary.location);
          break;
        default:
          VCL_UNREACHABLE();
          return false;
...
bool Compiler::Compile(const ast::Stmt& stmt) {
  if (!Compile(*stmt.expr)) return false;
...
bool Compiler::Compile(const ast::Prefix& pref) {
  return CompilePrefixList(pref.location, pref.list, pref.list.size());
}
...
bool Compiler::CompileComponent(const vcl::util::CodeLocation& loc,
  ...
  switch (comp.tag) {
    ...
    case ast::Prefix::Component::INDEX: {
      if (!Compile(*comp.expression)) return false;
      m_procedure->code_buffer().Emit(loc, BuildBytecode<CATEGORY_INDEX, OP>());
      break;
    }
    case ast::Prefix::Component::ATTRIBUTE: {
      int id = CompileString(loc, comp.var);
...
bool Compiler::Compile(const ast::Unset& unset) {
  ...
  if (lhs.type == ast::LeftHandSide::VARIABLE) {
    ...
    if (!CompileComponent<OP_UNSET>(unset.location, comp)) return false;
  }
  return true;
}
...
bool Compiler::Compile(const ast::Set& set) {
  ...
  if (lhs.type == ast::LeftHandSide::VARIABLE) {
    ...
    switch (set.op) {
      case TK_ASSIGN:
        if (!CompileComponent<OP_SET>(set.location, last)) return false;
        break;
      case TK_SELF_ADD:
        if (!CompileComponent<OP_SADD>(set.location, last)) return false;
        break;
      case TK_SELF_SUB:
        if (!CompileComponent<OP_SSUB>(set.location, last)) return false;
        break;
      case TK_SELF_MUL:
        if (!CompileComponent<OP_SMUL>(set.location, last)) return false;
        break;
      case TK_SELF_DIV:
        if (!CompileComponent<OP_SDIV>(set.location, last)) return false;
        break;
      case TK_SELF_MOD:
        if (!CompileComponent<OP_SMOD>(set.location, last)) return false;
        break;
      default:
        return false;
    }
...
bool Compiler::CompileFuncCall(const ast::FuncCall& fc) {
  ...
  if (intrinsic == INTRINSIC_FUNCTION_UNKNOWN) {
    if (fc.name && !CompileVariable(fc.location, fc.name)) return false;
  }
...
bool Compiler::Compile(const ast::If& node) {
  ...
  // 1. Generate the first If
  {
    ...
    if (node.branch_list.size() > 1) exit_list.push_back(__ jmp(node.location));
  }
...
bool Compiler::Compile(const ast::For& node) {
  ...
  // 2. Loop code generation
  {
    ...
    if (!BytecodeBuffer::CheckOperand(
            static_cast<uint32_t>(m_procedure->code_buffer().position()))) {
      ReportError(node.location,
                  "The loop body is *too* large, the generated byte "
    ...
    // by the scopes rules. So we simply do not need to anything to that
    // iterator
    // object at all.
...
bool Compiler::Compile(const CompilationUnit& cu) {
  ...
  for (size_t i = 0; i < cu.size(); ++i) {
    ...
    if (stmt.code.which() == CompilationUnit::STMT_AST) {
      if (!Compile(*boost::get<const ast::AST*>(stmt.code))) return false;
    } else {

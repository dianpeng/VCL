...
// 2. Reshuffle the constructs to allow code generator(compiler) generate correct
...
class CompilationUnit {
 ...
 public:
  ...
  struct Statement {
    ...
    Statement( const ast::AST* a , uint32_t si ):
      source_index(si),
      code(a)
    {}
  };
 ...
 public:
  ...
  Statement& Index( size_t index ) {
    return m_statement[index];
  }
  const Statement& Index( size_t index ) const {
    return m_statement[index];
  }

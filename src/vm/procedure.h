...
// Procedure is the implementation of SubRoutine object. SubRoutine object is
// just
// a shim layer of procedure object. A procedure object is bounded to use
// ImmutableGC
// and it stores inside of CompiledCode object. Whenever a new Context is
// created,
// then a SubRoutine object will be created with ContextGC and rerefence to this
// procedure object. A procedure object is sort of permenent since it represents
// a
// compiled procedure defined in code. A SubRoutine object represents its
// runtime.
class Procedure {
 public:
 ...
 public:  // Protocol related
  ...
  size_t argument_size() const { return m_arg_count; }

  // Code buffer
  BytecodeBuffer& code_buffer() { return m_code_buffer; }

  const BytecodeBuffer& code_buffer() const { return m_code_buffer; }
 ...
 public:
  template <typename T>
  typename detail::type_to_rtype<T>::return_type Index(int index) const {
  ...
  int32_t IndexInteger(int index) const { return Index<int32_t>(index); }

  double IndexReal(int index) const { return Index<double>(index); }
  ...
  vcl::ACL* IndexACL(int index) const { return Index<vcl::ACL*>(index); }

...
const Lexeme& Lexer::LexCode() {
  ...
  do {
    ...
    switch (c) {
      case '+':
        return Lookahead(TK_SELF_ADD, '=', TK_ADD);
      case '-':
        return Lookahead(TK_SELF_SUB, '=', TK_SUB);
      case '*':
        return Lookahead(TK_SELF_MUL, '=', TK_MUL);
      case '/': {
        int nc = src[m_pos + 1];
        if (nc == '/') {
          SkipLineComment();
      ...
      case '%':
        return Lookahead(TK_SELF_MOD, '=', TK_MOD);
      case '~':
        YIELD(TK_MATCH, 1);
      case '!':
        return Lookahead(TK_NOT_MATCH, '~', TK_NE, '=', TK_NOT);
      case '#':
        SkipLineComment();
        continue;
      case '=':
        return Lookahead(TK_EQ, '=', TK_ASSIGN);
      case '<':
        return Lookahead(TK_LE, '=', TK_LT);
      case '>':
        return Lookahead(TK_GE, '=', TK_GT);
      case '&':
        return Lookahead(TK_AND, '&', TK_ERROR);
      case '|':
        return Lookahead(TK_OR, '|', TK_ERROR);
      case ':':
        return Lookahead(TK_FIELD, ':', TK_COLON);
      case ';':
        YIELD(TK_SEMICOLON, 1);
      case ',':
        YIELD(TK_COMMA, 1);
      case '.':
        YIELD(TK_DOT, 1);
...
bool Lexer::TryTokenAsExtendedVar() {
  ...
  if (IsSymbolInitChar(c)) {
    while (IsExtendedVarChar(src[start])) ++start;
    m_lexeme.token_length = (start - m_pos);
    ...
    LexerError(
        "Expect a variable name,you are allowed to put dash inside of "
...
const Lexeme& Lexer::LexNumPrefix() {
  ...
  while (std::isdigit(src[start])) ++start;
  ...
  switch (src[start]) {
    ...
    case 's':
      return LexDuration<DURATION_SEC>(start + 1, start);
    case 'm': {
      int nc = src[start + 1];
      if (nc == 's')
        ...
        int nnc = src[start + 2];
        if (nnc == 'n') return LexDuration<DURATION_MIN>(start + 3, start);
      }
    } break;
    case 'g':
  ...
  try {
    m_lexeme.value =
        boost::lexical_cast<int32_t>(m_source.substr(m_pos, start - m_pos));
  } catch (...) {
...
const Lexeme& Lexer::LexReal(int pos) {
  ...
  if (start == pos + 1) {
    return LexerError(
        "real number is in ill-formatted, expect more digits "
        "after the dot");
  }
  ...
  try {
    m_lexeme.value =
        boost::lexical_cast<double>(m_source.substr(m_pos, start - m_pos));
  } catch (...) {
...
const Lexeme& Lexer::LexLineStr() {
  ...
  do {
    ...
    if (c == '\\') {
      ...
      return LexerError(
          "single line string has line break character ,"
...
// a flag called is_symbol. This flag will tell the parser that whether they
// want
...
const Lexeme& Lexer::LexVarOrKeyword() {
  ...
  switch (c) {
    case 'a':
      if (CHECK_KEYWORD(len, src + m_pos + 1, "cl")) YIELD(TK_ACL, 3);
      return LexVar(len);
    case 'b':
      if (CHECK_KEYWORD(len, src + m_pos + 1, "reak")) YIELD(TK_BREAK, 5);
      return LexVar(len);
    case 'c':
      ...
      return LexVar(len);
    case 'd':
      if (CHECK_KEYWORD(len, src + m_pos + 1, "eclare")) YIELD(TK_DECLARE, 7);
      return LexVar(len);
    case 'e':
    ...
    case 'f':
      ...
      return LexVar(len);
    case 'g':
      if (CHECK_KEYWORD(len, src + m_pos + 1, "lobal")) YIELD(TK_GLOBAL, 6);
      return LexVar(len);
    case 'i':
    ...
    case 'n':
      ...
      return LexVar(len);
    case 'r':
      if (CHECK_KEYWORD(len, src + m_pos + 1, "eturn")) YIELD(TK_RETURN, 6);
      return LexVar(len);
    case 's':
      ...
      return LexVar(len);
    case 't':
      if (CHECK_KEYWORD(len, src + m_pos + 1, "rue")) YIELD(TK_TRUE, 4);
      return LexVar(len);
    case 'u':
      if (CHECK_KEYWORD(len, src + m_pos + 1, "nset")) YIELD(TK_UNSET, 5);
      return LexVar(len);
    case 'v':
      if (CHECK_KEYWORD(len, src + m_pos + 1, "cl")) YIELD(TK_VCL, 3);
      return LexVar(len);
    default:
...
const Lexeme& Lexer::LexVar(int start) {
  ...
  while (IsSymbolChar(src[pos])) ++pos;

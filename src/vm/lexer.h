...
// But by this we slightly change the semnatic of the grammar , the keyword
// cannot
// be used as symbol now. However I don't think any one will use keyword as
// variable
// name.
//
// Order matters !!!!
...
enum Token {
#define __(A, B) A,
  VCL_TOKEN_LIST(__) SIZE_OF_VCL_TOKENS
#undef __  // __
};

const char* GetTokenName(Token tk);

inline bool TokenIsBinaryOperator(Token tk) { return tk >= 0 && tk <= TK_OR; }
...
struct Lexeme {
  ...
  const std::string& string() const { return boost::get<std::string>(value); }
  ...
  bool is_symbol;
  Lexeme()
      : token(SIZE_OF_VCL_TOKENS), value(), token_length(0), is_symbol(false) {}
};

class Lexer {
 public:  // Static interfaces
  static bool IsSymbolChar(int c) { return std::isalnum(c) || c == '_'; }

  static bool IsSymbolInitChar(int c) { return std::isalpha(c) || c == '_'; }

  static bool IsExtendedVarChar(int c) { return IsSymbolChar(c) || c == '-'; }
 ...
 private:
  ...
  // Order matters
  enum { SIZE_B = 0, SIZE_KB, SIZE_MB, SIZE_GB };
  template <int TYPE>
  const Lexeme& LexSize(int, int);
  template <int TYPE>
  const Lexeme& LexSizeImpl(int, int);
  // Order matters
  enum { DURATION_MSEC = 0, DURATION_SEC, DURATION_MIN, DURATION_HOUR };
  template <int TYPE>
 ...
 private:
  ...
  enum { LEXER_STATE_NORMAL, LEXER_STATE_STRING_INTERPOLATION };
  ...
  // Flag to tell whether we are in code segment inside of the string
  // interpolation
...
const Lexeme& Lexer::LexDurationImpl(int start, int digit) {
  ...
  try {
    value =
        boost::lexical_cast<uint32_t>(m_source.substr(m_pos, (digit - m_pos)));
  } catch (...) {
    return LexerError(
        "cannot convert duration quantity component %s to uint32_t!",
...
const Lexeme& Lexer::LexSizeImpl(int start, int digit) {
  ...
  switch (TYPE) {
    ...
    case SIZE_KB:
      ...
      break;
    case SIZE_MB:
      size.megabytes = value;
      break;
    case SIZE_GB:
      size.gigabytes = value;
      break;
    default:
      VCL_UNREACHABLE();
      break;
  ...
  if (SIZE_B != TYPE) {
    ...
    switch (src[pos]) {
      ...
      case 'b':
      case 'B':
        if (SIZE_B < TYPE) return LexSizeImpl<SIZE_B>(pos + 1, pos);
        break;
      default:
        break;
    }

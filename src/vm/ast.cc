...
std::ostream& Serialize( const AST& node , int indent , std::ostream* output ) {
  switch(node.type) {
#define __(A,B,C) case AST_##A: return Serialize( static_cast<const B&>(node) \
    , indent , output );

    VCL_AST_TYPE(__)

#undef __ // __
    default: VCL_UNREACHABLE(); return *output;
  }
}
...
std::ostream& Serialize( const Import& import, int indent , std::ostream* output ) {
  VCL_UNUSED(indent);
...
std::ostream& Serialize( const LexScope& lscope , int indent , std::ostream* output ) {
...
std::ostream& Serialize( const Extension& ext , int indent , std::ostream* output ) {
...
std::ostream& Serialize( const Continue& node , int indent , std::ostream* output ) {
...
std::ostream& Serialize( const For& node , int indent , std::ostream* output ) {
  ...
  if(node.key) (*output) << node.key->data();
  else (*output) << '_';
  (*output) << ',';

  if(node.val) (*output) << node.val->data();
  else (*output) << '_';
  (*output) << ':';
...
std::ostream& Serialize( const Prefix& p , int indent , std::ostream* output ) {
  ...
  *output<<*(p.list.First().var);
  for( size_t i = 1; i < p.list.size() ; ++i ) {
    ...
    switch(c.tag) {
      case Prefix::Component::CALL: Serialize(*c.funccall,indent,output); break;
      case Prefix::Component::INDEX:
        *output<<'[';
        Serialize(*c.expression,indent,output)<<']';
        break;
      case Prefix::Component::DOT: *output<<'.'<<*c.var; break;
      case Prefix::Component::ATTRIBUTE: *output <<':'<<*c.var; break;
...
std::ostream& Serialize( const FuncCall& fc , int indent ,
    std::ostream* output ) {
  if(fc.name) {
...
std::ostream& Serialize( const Integer& i , int indent ,
    std::ostream* output ) {
  VCL_UNUSED(indent);
  return *output<<i.value;
}

std::ostream& Serialize( const Real& r , int indent ,
        std::ostream* output ) {
  VCL_UNUSED(indent);
  return *output<<r.value;
}

std::ostream& Serialize( const Boolean& b , int indent ,
    std::ostream* output ) {
  VCL_UNUSED(indent);
  return *output<<std::boolalpha<<b.value<<std::noboolalpha;
}

std::ostream& Serialize( const Null& n , int indent ,
    std::ostream* output ) {
  VCL_UNUSED(indent);
  VCL_UNUSED(n);
  return *output << "null";
...
std::ostream& Serialize( const String& s , int indent ,
    std::ostream* output ) {
  VCL_UNUSED(indent);
  return *output << '"' << Escape(*s.value) << '"';
}

std::ostream& Serialize( const Variable& v , int indent ,
    std::ostream* output ) {
  VCL_UNUSED(indent);
  return *output << *v.value;
}

std::ostream& Serialize( const Duration& d , int indent ,
    std::ostream* output ) {
  VCL_UNUSED(indent);
  return *output << d.value;
}

std::ostream& Serialize( const Size& s , int indent ,
    std::ostream* output ) {
  VCL_UNUSED(indent);
...
std::ostream& Serialize( const File& f , int indent ,
    std::ostream* output ) {
...
#define __(A,B,C) case AST_##A: return C;
  switch(type) {
    VCL_AST_TYPE(__)
    default: return NULL;
  }

...
const MethodStatus MethodStatus::kUnimplemented(MethodStatus::METHOD_UNIMPLEMENTED);
...
template< typename T > bool OperatorImpl( Context* context ,
                                          const Value& left ,
                                          const Value& right,
                                          Value* output ,
                                          MethodStatus* status ) {
  ...
  switch(left.type()) {
    ...
    case TYPE_SIZE:
    case TYPE_DURATION:
      return MethodStatus::NewUnimplemented("Arithmatic operator cannot work between type "
          "%s and %s", left.type_name() , right.type_name());
    case TYPE_BOOLEAN:
      {
        if(right.IsReal()) {
          if(!handler.CheckRHS(right.GetReal())) {
...
bool Value::ConvertToString( Context* context , const Value& value,
  ...
  switch(value.type()) {
    case TYPE_INTEGER:
      ...
      break;
    case TYPE_REAL:
      *output = context->gc()->NewString(vcl::util::RealToString(
            value.GetReal()));
      break;
    case TYPE_NULL:
    ...
    case TYPE_DURATION:
      *output = (context->gc()->NewString(
      ...
      break;
    case TYPE_SIZE:
      *output = context->gc()->NewString(vcl::util::Size::ToString(value.GetSize()));
      break;
    default:
...
typedef MethodStatus (Object::*CompareCallback)(
    Context* , const Value& , bool* ) const;
...
MethodStatus ComparisonOp( Context* context , CompareCallback callback,
  ...
  switch(left.type()) {
    ...
    case TYPE_REAL:
      {
        double value;
        if(!(result = right.ToReal(context,&value)))
          return result;
    ...
    case TYPE_NULL:
      if(boost::is_same<T,EqualOp>::value ||
        ...
        *output = handler.Do( true , right.IsNull() );
        ...
        return MethodStatus::NewFail("null only support comparison operator: "
                            "==/!=");
      }
    case TYPE_BOOLEAN:
      {
    ...
    case TYPE_SIZE:
    case TYPE_DURATION:
      return MethodStatus::NewFail("size and duration/time doesn't support "
                          "comparison operator!");
    default:
...
MethodStatus Value::Less( Context* context , const Value& value ,
    bool* output ) const {
  return ComparisonOp<LessOp>(context,&Object::Less,
      *this,value,output);
}

MethodStatus Value::LessEqual(Context* context , const Value& value ,
    bool* output ) const {
  return ComparisonOp<LessEqualOp>(context,&Object::LessEqual,
      *this,value,output);
}

MethodStatus Value::Greater(Context* context, const Value& value ,
    bool* output ) const {
  return ComparisonOp<GreaterOp>(context,&Object::Greater,
      *this,value,output);
}

MethodStatus Value::GreaterEqual(Context* context, const Value& value ,
    bool* output ) const {
  return ComparisonOp<GreaterEqualOp>(context,&Object::GreaterEqual,
      *this,value,output);
}

MethodStatus Value::Equal( Context* context , const Value& value ,
    bool* output ) const {
  return ComparisonOp<EqualOp>(context,&Object::Equal,
      *this,value,output);
}

MethodStatus Value::NotEqual(Context* context, const Value& value ,
    bool* output ) const {
  return ComparisonOp<NotEqualOp>(context,&Object::NotEqual,
      *this,value,output);
}
...
MethodStatus String::Add( Context* context , const Value& value ,
  ...
  MethodStatus result;
  std::string temp;
  if(!(result=value.ToString(context,&temp)))
    return result;
...
MethodStatus String::Match( Context* context , const Value& value ,
  ...
  if(value.IsString()) {
    ...
    if(!(result = str->Match(context,*this,output)))
      return result;
    return MethodStatus::kOk;
  } else {
    return MethodStatus::NewFail("regex matching must applied on type string,but "
                                 "got type %s",value.type_name());
  }
}
...
MethodStatus StringCompareOp( Context* context , const String& left ,
  ...
  if(right.IsString()) {
    ...
    *output = handler.Do(left,*right_string);
    ...
    std::string temp;
    if(!(result = right.ToString(context,&temp)))
      return result;
...
MethodStatus String::Regex::Match(Context* context, const String& pattern,
  ...
  int stat[3];
  int ret = pcre_exec(ctx, extra, string.data(), string.size(),
      0, 0, stat, 3);
  *output = (ret >= 0);
  return MethodStatus::kOk;
}
...
// ACL Implementation
// =======================================================================
bool ACL::Match( const in_addr& addr ) const {
  return m_impl->Match(addr);
}

bool ACL::Match( const in6_addr& addr ) const {
  return m_impl->Match(addr);
}

bool ACL::Match( const char* addr ) const {
  return m_impl->Match(addr);
}
...
// ========================================================================
MethodStatus Function::ToDisplay( Context* context ,
    std::ostream* output ) const {
  VCL_UNUSED(context);
...
// ========================================================================
MethodStatus Action::ToDisplay( Context* context ,
    std::ostream* output ) const {
  VCL_UNUSED(context);
...
// ========================================================================
MethodStatus Module::ToDisplay( Context* context ,
    std::ostream* output ) const {
...
const std::string& SubRoutine::name() const {
    ...
    switch(obj->type()) {
      ...
      case TYPE_FUNCTION:
        *output<<i<<". func("<<static_cast<Function*>(obj)->name()<<")\n";
        break;
      case TYPE_EXTENSION:
        *output<<i<<". ext("<<static_cast<Extension*>(obj)->extension_name()<<")\n";
        break;
      case TYPE_ACTION:
        *output<<i<<". act("<<static_cast<Action*>(obj)->action_code_name()<<")\n";
        break;
      case TYPE_MODULE:
...
MethodStatus Context::Construct() {
  Value result;
  SubRoutine* e = InternalAllocator(&m_gc).NewSubRoutine(
      m_compiled_code->entry());
...
MethodStatus Context::Invoke( SubRoutine* sub_routine ,
    const std::vector<Value>& argument , Value* output ) {
  MethodStatus result;
...
MethodStatus Context::Invoke( SubRoutine* sub_routine , const Value& a1 ,
    const Value& a2 , const Value& a3 , const Value& a4 , Value* output ) {
...
MethodStatus Context::Invoke( SubRoutine* sub_routine , const Value& a1 ,
    const Value& a2 , const Value& a3 , const Value& a4 ,
    const Value& a5 , const Value& a6 , const Value& a7 , Value* output ) {
...
size_t Context::GetArgumentSize() const {
  return m_runtime->GetArgumentSize();
}
...
bool Context::is_yield() const {
  return m_runtime->is_yield();
}
...
uint32_t CompiledCode::AddSourceCodeInfo(
  ...
  for( size_t i = 0 ; i < m_source_code_list.size() ; ++i ) {
    ...
    if(info->file_path == sci->file_path)
...
boost::shared_ptr<SourceCodeInfo>
CompiledCode::IndexSourceCodeInfo( uint32_t index ) const {
...
boost::shared_ptr<CompiledCode> Engine::LoadString( const std::string& filename ,

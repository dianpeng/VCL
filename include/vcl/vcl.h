...
inline const char* GetValueTypeName( ValueType val ) {
  switch(val) {
#define __(A,B,C) case TYPE_##A : return C;
    VCL_VALUE_TYPE_LIST(__)
    default: return NULL;
#undef __ // __
  }
...
// function handler.
// 1. OK  , indicate the function execute correctly and finish with return value.
...
// 5. Terminate , indicate this function will be terminated right after calling it.
//    This is used to indicate to the user that we are done via a terminate action.
//    VCL script subroutine can generate a terminate via vcl return statement, and
...
class MethodStatus VCL_FINAL {
  ...
  struct Unimplemented { std::string description; };
 ...
 public:
  ...
  bool is_unimplemented() const { return m_storage.which() == METHOD_UNIMPLEMENTED; }
  ...
  void set_yield( const boost::any& value ) {
    m_storage = Yield(value);
  }
 ...
 private:
  ...
  MethodStatus( const boost::any& data ):
    m_storage(Yield(data))
  {}
...
// The iterator of std::list has a nice feature that delete other node in std::lisst
...
// }
//
// The above code will basically mark the string object you created via NewString
// to be reachable , until goes out of the lexical scope. So within that scope,
// user are always able to use that string without GC collecting for that string.
//
// To prevent very hard to debug errors like :
//
// my_object->SetProperty(context,*gc->NewString("A"),Value(gc->NewList()));
//
// The above code is very simple but prone to bug, since while evaluating gc->NewString,
// the gc->NewList 's return object can be collected because it is unreachable during
// that GC cycle. User should *never* write above like code , the safe way to go is :
//
// Handle<String> key( gc->NewString("A") , &gc );
// Handle<List> value( gc->NewList() , &gc );
// my_object->SetProperty(context,*key,Value(value.get()));
//
// So the conclusion is, when you write C++ code that is allocating script side object
// via GC interfaces, please *ALWAYS* use Handle object.

template< typename T > class Handle VCL_FINAL {
 public:
  ...
  T& operator * () { DCHECK(m_value); return *m_value; }
  const T& operator * () const { DCHECK(m_value); return *m_value; }
  T* operator -> () { DCHECK(m_value); return m_value; }
  const T* operator ->() const { DCHECK(m_value); return m_value; }
...
#define __(A,B,C) bool Is##B() const { return m_type == TYPE_##A; }
  VCL_VALUE_TYPE_LIST(__)
#undef __ // __
  ...
  const char* type_name() const {
    return GetValueTypeName(m_type);
  }
 public:
...
template<> class Handle<Value> VCL_FINAL {
 public:
...
#undef __ // __
 ...
 public:
  ...
  //
  // For primitive types and string , some arithmatic operation is already provided.

  // Property Operations
  virtual MethodStatus GetProperty( Context* , const String& key , Value* ) const;
  virtual MethodStatus SetProperty( Context* , const String& key , const Value& value );
 ...
 protected:
  ...
  // }
  //
  // Notes: Here when we try to mark String and Extension object, the function we
  ...
  bool is_white() const {
    return m_gc_state == GC_WHITE;
  }

  bool is_black() const {
    return m_gc_state == GC_BLACK;
  }

  bool is_gray() const {
    return m_gc_state == GC_GRAY;
  }
...
class StringDict VCL_FINAL {
 ...
 public:
  ...
  StringDict( size_t cap = kDefaultCapSize ):
    m_entry(),
    m_used(0),
    m_size(0)
  { DCHECK( !(cap & (cap-1)) ); m_entry.resize(cap); }
  ...
  template< typename ALLOC >
  void InsertOrUpdate( ALLOC* gc , const std::string& key ,
      const T& value ) {
 ...
 public: // GC
  ...
  typedef IteratorImpl< typename HashVector::iterator , ValueType >
    Iterator;
  ...
  Iterator Begin() {
    return Iterator(m_entry.begin(),m_entry.end());
  }
  Iterator End() {
    return Iterator(m_entry.end(),m_entry.end());
  }
...
class Extension : public Object {
 ...
 protected:
  ...
  Extension( const std::string& ext_name ):
    Object(TYPE_EXTENSION),
    m_extension_name(ext_name)
  {}
...
// Extension Factory
// Extension factory are accossiated factory class for certain type of extension.
...
// A SubRoutine is a defined script side routine/function in script. User is allowed
// to invoke it basically for customization reason. All SubRoutine are GCed objects
...
// is allowed to use it call a script side defined function and perform customized
// logic afterwards.
...
class GC {
 protected:
  ...
  void ForceCollect() { ++m_gc_times; Mark(); Collect(); }
  ...
  size_t next_gc_trigger() const {
    return m_next_gc;
  }

  double gc_ratio() const {
    return m_gc_ratio;
  }
  ...
  void set_next_gc_trigger( size_t gc_trigger ) {
    m_next_gc = gc_trigger;
  }
 ...
 public: // General purpose object that user could use to allocate object
  ...
  T* New( const A1& a1 , const A2& a2 , const A3& a3 ,
          ...
          const A7& a7 ) {
    BOOST_STATIC_ASSERT( boost::is_base_of<Object,T>::value );
    return LinkObject<T>( ::new (Malloc(sizeof(T))) T(a1,a2,a3,a4,a5,a6,a7) );
  }
  ...
  T* New( const A1& a1 , const A2& a2 , const A3& a3 ,
    ...
    BOOST_STATIC_ASSERT( boost::is_base_of<Object,T>::value );
    return LinkObject<T>( ::new (Malloc(sizeof(T))) T(a1,a2,a3,a4,a5,a6,a7,a8) );
  }
...
// A specific GC that is sololy used by *Context* object in an isolated execution
...
class Environment {
 public:
  ...
  size_t GlobalVariableSize() const {
    return m_gvar_map.size();
  }
 protected:
...
// execution needs to existed, the CompiledCode must existed. Each context object
...
class Context VCL_FINAL : public detail::Environment<Context,ContextGC> {
 ...
 public: // Yield and Resume
  ...
  // which basically allow the user to execute virtual machine code in a preemptive
  // fashion.
  //
 ...
 public:
  ...
  MethodStatus Invoke( SubRoutine* , const Value& , const Value& ,
                                     const Value& , Value* );
...
inline const char* MethodStatus::status_name() const {
  switch(status()) {
    case METHOD_OK: return "ok";
    case METHOD_TERMINATE: return "terminate";
    case METHOD_FAIL: return "fail";
    case METHOD_YIELD: return "yield";
    case METHOD_UNIMPLEMENTED: return "unimplemented";
    default: VCL_UNREACHABLE(); return NULL;
  }
}
...
inline void MethodStatus::set_status( int s ) {
  switch(s) {
    case METHOD_OK : m_storage = Ok(); break;
    case METHOD_TERMINATE: m_storage = Terminate(); break;
    case METHOD_FAIL:m_storage = Fail(); break;
    case METHOD_YIELD:m_storage= Yield(); break;
    case METHOD_UNIMPLEMENTED: m_storage = Unimplemented(); break;
...
inline Handle<T>::Handle( T* value , GC* gc ):
  m_value(value),
  m_gc(gc),
  m_iterator( gc->AddRoot(value) )
{}

template< typename T >
inline Handle<T>::~Handle() { if(m_value) m_gc->RemoveRoot(m_iterator); }
...
inline Value::Value( List* value ):
  m_value(value),
  m_type(TYPE_LIST)
{}

inline Value::Value( Dict* value ):
  m_value(value),
  m_type(TYPE_DICT)
{}

inline Value::Value( ACL* value ):
  m_value(value),
  m_type (TYPE_ACL)
{}

inline Value::Value( Function* value ):
  m_value(value),
  m_type (TYPE_FUNCTION)
{}

inline Value::Value( Action* value ):
  m_value(value),
  m_type (TYPE_ACTION)
{}
...
inline Value::Value( Module* value ):
  m_value(value),
  m_type (TYPE_MODULE)
{}
...
inline Value::Value( Iterator* value ):
  m_value(value),
  m_type (TYPE_ITERATOR)
{}

template< typename E >
inline Value::Value( Handle<E>& h ):
  m_value(h.get()),
  m_type (h->type())
{}

template< typename E >
inline Value::Value( const Handle<E>& h ):
  m_value(h.get()),
  m_type (h->type())
{}
...
template< int OPTION >
typename StringDict<T,Hasher>::HashEntry*
StringDict<T,Hasher>::FindEntry( const char* string , uint32_t full_hash ) const {
...
template< typename T , typename Hasher >
bool StringDict<T,Hasher>::Insert( const String& string ,
    const T& value ) {
...
template< typename T , typename Hasher >
bool StringDict<T,Hasher>::Update( const String& string ,
    const T& value ) {
  uint32_t full_hash = HashKey(string.data(),string.size());
...
void StringDict<T,Hasher>::InsertOrUpdate( ALLOC* gc , const char* string ,
  ...
  if(entry->used && !entry->del) {
    ...
    String* key = gc->NewString(string); // Do it here since this prevent GC mark
...
T* StringDict<T,Hasher>::Find( const String& key ) const {
  HashEntry* entry = FindEntry<OPT_FIND>(key.data(),
...
T* StringDict<T,Hasher>::Find( const char* string ) const {
  HashEntry* entry = FindEntry<OPT_FIND>(string,
                                         HashKey(string,strlen(string)));
  return entry ? &(entry->pair.second) : NULL;
}
...
void StringDict<T,Hasher>::Rehash() {
  ...
  for( size_t i = 0 ; i < vec.size() ; ++i ) {
    ...
    HashEntry* new_entry = FindEntry<OPT_INSERT>(entry.pair.first->data(),
                                                 entry.full_hash);
...
template< typename T > struct DoGCMark {
  static void Do( T& value ) { value.Mark(); }
};

template< typename T > struct DoGCMark<T*> {
  static void Do( T* value ) { value->Mark(); }
};

template< typename T > struct DoGCMark< const T > {
...
template< typename T , typename GCType >
bool Environment<T,GCType>::RegisterExtensionFactory( const std::string& name ,
...
template< typename T , typename GCType >
Environment<T,GCType>::~Environment() { ExtensionMap::DoDelete(&m_ext_map); }

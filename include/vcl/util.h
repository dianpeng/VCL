...
struct Duration {
  ...
  Duration(uint32_t h = 0, uint32_t min = 0, uint32_t s = 0, uint32_t m = 0)
      : hour(h), minute(min), second(s), millisecond(m) {}

 public:
  inline Duration& operator+(const Duration&);
  inline Duration& operator-(const Duration&);
  bool operator==(const Duration& duration) const {
    return hour == duration.hour && minute == duration.minute &&
           second == duration.second && millisecond == duration.millisecond;
  }
...
struct Size {
  ...
  Size(uint32_t gb = 0, uint32_t mb = 0, uint32_t kb = 0, uint32_t b = 0)
 ...
 public:
  ...
  bool operator==(const Size& size) const {
    return bytes == size.bytes && kilobytes == size.kilobytes &&
           megabytes == size.megabytes && gigabytes == size.gigabytes;
  }
...
struct CodeLocation {
  ...
  CodeLocation() : line(0), ccount(0), position(0) {}

  CodeLocation(int l, int c, size_t p) : line(l), ccount(c), position(p) {}
};
...
// Operators ================================================================
inline std::ostream& operator<<(std::ostream& output, const CodeLocation& cl) {
...
inline std::ostream& operator<<(std::ostream& output, const Size& size) {
...
inline bool IsDirectorySeparator(int c) { return c == '/'; }

inline int GetDirectorySeparator() { return '/'; }
